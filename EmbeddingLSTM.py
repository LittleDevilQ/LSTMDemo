...
class lstm():
    ...
    def __init__(self, dataset, hyper_params):
        ...
        self.learning_rate = hyper_params[4]
        self.selected_feature = hyper_params[5]  # Initialize the selected features/初始化选择特征
        self.train_ratio = hyper_params[6]  # Initialize the splitted ratio of training/初始化训练集分割比例
        self.feature_num = hyper_params[7]  # Initialize the number of features/初始化特征数量
    ...
    def mape(self, scaler, trainPredict, testPredict):
        ...
        trainPredict_dataset_like[:, 0] = trainPredict[:, 1]
        ...
        precision = precision_score(self.y_test, trainPredict)
        recall = recall_score(self.y_test, trainPredict)
    ...
    def plot(self, scaler, trainPredict, testPredict):
        ...
        plt.savefig(r'D:\VSCode\CodeWorkplace\.vscode\result.png', dpi=900)  # 保存图片
...
if __name__ == "__main__":
    ...
    file = r'D:\VSCode\CodeWorkplace\.vscode\PC_dataset.csv'
    dataframe = pd.read_csv(file , header=None, index_col=None)
    dataset = dataframe.iloc[:, [0,1]].values
    ...
    num_neur = [128, 128]  # Number of layer and number of neurons in each layer/隐藏层数和各层神经元个数
    look_back = 64  # Length of windows/窗口长度
    epochs = 100  # Training times/训练次数
    batch_size = 10 # Batch size/批数大小
    learning_rate = 0.001 #学习率
    select_feature = [1, 1]  # Selected features list/被选择特征列表
    train_ratio = 0.70  # Splitted ratio of training data set/训练集分割比例
    feature_num = dataset.shape[1]  # Feature number+y/特征数量+1,也将预测项作为特征
    # Hyper-parameter list/超参数列表
    hyper_params = [num_neur, look_back, epochs, batch_size, learning_rate, select_feature, train_ratio, feature_num]
